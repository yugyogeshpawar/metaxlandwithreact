...
/** @typedef {import('./grid').GridOverEvent} GridHoverEvent */
/** @typedef {import('./grid').GridMoveEvent} GridMoveEvent */



export const GRID_CLICK_EVENT = 'grid.click';
export const GRID_OVER_EVENT = 'grid.mouseover';
export const GRID_MOVE_EVENT = 'grid.mousemove';
...
export class Grid {
  ...
  constructor(config) {
    ...
    this.onMapOverBound = this.onMapHover.bind(this);
  ...
  onAdd(map) {
    ...
    this.map.on('mouseover', this.onMapOverBound);
    this.map.on('mousemove', this.onMapMoveBound);
  ...
  onRemove() {
    ...
    this.map.off('over', this.onMapOverBound);
  ...
  onMapHover(event) {
    ...
    /** @type {GridHoverEvent} */
    const event3 = { bbox };
    this.map.fire(GRID_OVER_EVENT, event3);
  ...
  onMapMove(event) {
    ...
    /** @type {GridMoveEvent} */
    const event3 = { bbox };
    this.map.fire(GRID_MOVE_EVENT, event3);
  }
}